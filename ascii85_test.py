from ascii85 import decode, decodeTuple
...
def test_decode1():
    assert decode(leviathan_encoded) == leviathan_decoded

def test_decode2():
    for (plain, encoded) in tuples:
        assert plain == decode('<~' + encoded + '~>')

def test_decodeTuple():
    for (plain, encoded) in tuples:
        assert plain == decodeTuple(encoded.ljust(5, 'u'))[0:len(plain)]
